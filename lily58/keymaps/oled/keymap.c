...
const uint16_t PROGMEM test_combo1[] = {KC_P, KC_Y, COMBO_END};
const uint16_t PROGMEM test_combo11[] = {KC_R, KC_T, COMBO_END};
const uint16_t PROGMEM test_combo2[] = {KC_U, KC_I, COMBO_END};
const uint16_t PROGMEM test_combo22[] = {KC_F, KC_G, COMBO_END};
combo_t key_combos[4] = {
    COMBO(test_combo1, KC_PGUP),
    COMBO(test_combo11, KC_PGUP),
    COMBO(test_combo2, KC_PGDOWN),
    COMBO(test_combo22, KC_PGDOWN),
};
 ...
 * `-----------------------------------------/       /     \      \-----------------------------------------'
 *                   | LAlt | LGUI |LCTRL | /Space  /       \ENTER \  |BK/RA |RAISE | LOWER|
 *                   |      |      |      |/       /         \      \ |      |      |      |
 *                   `----------------------------'           '------''--------------------'
 */
 ...
 [_QWERTY] = LAYOUT( \
  ...
  KC_LSFT,  KC_Z,   KC_X,    KC_C,    KC_V,    KC_B, KC_GRV,  LT(KC_DEL,KC_F13),  KC_N,    KC_M,    KC_COMM, KC_DOT,  KC_SLSH,  KC_BSLASH, \
...
[_LOWER] = LAYOUT( \
  ...
  _______, KC_QUES, _______, _______, _______, _______, DF(_DVORAK), DF(_QWERTY), KC_PEQL, KC_UNDS, KC_PLUS, KC_LCBR, KC_RCBR, KC_PIPE, \
 ...
 * |   `  |   1  |   2  |   3  |   4  |   5  |                    |   6  |   7  |   8  |   9  |   0  |      |
 * |------+------+------+------+------+------|                    |------+------+------+------+------+------|
 * |  F1  |  F2  |  F3  |  F4  |  F5  |  F6  |-------.    ,-------| HOME | Left | Down |  Up  |Right |      |
 * |------+------+------+------+------+------|   [   |    |    ]  |------+------+------+------+------+------|
...
[_RAISE] = LAYOUT( \
  _______, _______, _______, _______, _______, _______,                     _______, _______, _______, _______, _______, _______, \
  KC_F1,   KC_F2,   KC_F3,   KC_F4,   KC_F5,   KC_F6,                     KC_F7,   KC_F8,   KC_F9,   KC_F10,  KC_F11,  KC_F12, \
  _______, _______, KC_MPRV,KC_MPLY,KC_MNXT,KC_RSFT, KC_HOME, KC_LEFT, KC_DOWN, KC_UP,   KC_RGHT, XXXXXXX, \
  _______, _______, KC__MUTE,KC__VOLDOWN,KC__VOLUP, _______,   _______, _______,  KC_END, XXXXXXX, KC_PGDOWN,  KC_PGUP, KC_RBRC, KC_BSLS, \
                             _______, _______, _______,  _______, _______,  _______, _______, _______ \
),
/* DVORAK
 * ,-----------------------------------------.                    ,-----------------------------------------.
 ...
 * |LShift|   Z  |   X  |   C  |   V  |   B  |-------|    |-------|   N  |   M  |   ,  |   .  |   /  |  |   |
 * `-----------------------------------------/       /     \      \-----------------------------------------'
 *                   | LAlt | LGUI |LOWER | /Space  /       \ENTER \  |BAKSP |RAISE | ANOTHER LAYER? |
 *                   |      |      |      |/       /         \      \ |      |      |      |
 *                   `----------------------------'           '------''--------------------'
 */
 ...
 [_DVORAK] = LAYOUT( \
  ...
  KC_LSFT, KC_SCOLON,KC_Q,   KC_J,    KC_K,    KC_X, KC_GRV,LT(_LOWER,KC_F13),    KC_B,    KC_M,    KC_W,    KC_V,    KC_Z,   KC_BSLASH, \
              LT(_LOWER,KC_LALT), KC_LGUI, KC_LCTRL, KC_SPC, KC_ENT, LT(_RAISE,KC_BSPC), MO(_RAISE), MO(_LOWER) \
)
...
layer_state_t layer_state_set_user(layer_state_t state) {
  return update_tri_layer_state(state, _LOWER, _RAISE, _DVORAK);
}
